...
#        self.category_tmp_httpaceproxy = None
    ...
    def checkZones(self):
        ...
        if userinfo["zone_id"] != int(defines.getSetting("proxy_zoneid", "0")) + 1:
            print "user zone_id != setting zone_id"
            defines.setSetting("proxy_zoneid", "%s" % (userinfo["zone_id"] - 1))

        rzones = defines.GET_JSON("http://1ttvxbmc.top/v3/get_zones.php?session=%s&typersult=json" % self.session)["data"]
        rzones = [z["name"] for z in rzones]
        rzones = "|".join(rzones)
        lzones = defines.getSettingDef("proxy_zoneid", "TSProxy")["values"]
    ...
    def curl_gzip_pomoyka(self, request, path_playlist_pomoyka):
        ...
        if response.info().get('Content-Encoding') == 'gzip':
            print 'lysyi ENGINE_POMOYKA download zip playlist'
            buf = StringIO(response.read())
            f = gzip.GzipFile(fileobj=buf)
            with open(path_playlist_pomoyka, 'w') as playlist:
                lines = f.read()
